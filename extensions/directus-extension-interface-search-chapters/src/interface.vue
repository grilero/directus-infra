...
const primaryKey = fieldsStore.getPrimaryKeyFieldForCollection(
    ...
    const response = await api.get(
      `/items/${relatedCollection.value.collection}`,
      {
        params: {
          ...
          filter: props.filter,
          search:
            searchQuery.value && searchQuery.value != props.value
              ? searchQuery.value
              : "",
        },
      },
...
function useRelation() {
  const relation = computed(() => {
    return relationsStore.getRelationsForField(
      props.collection,
      props.field,
    )?.[0];
  });
